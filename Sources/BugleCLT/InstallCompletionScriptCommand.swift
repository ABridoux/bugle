...
struct InstallCompletionScriptCommand: ParsableCommand {
    ...
    static let configuration = CommandConfiguration(
        ...
        abstract: "Install Bugle completion script in the right directoy",
        discussion: "See \(argumentParserDocumentation)",
        shouldDisplay: false)

    var fileManager: FileManager { .default } // computer for `Decodable` synthesis (with `ParsableCommand`)
    ...
    private func installScript(for shell: CompShell) throws {
        ...
        let script = BugleCommand.completionScript(for: shellType.shell)
